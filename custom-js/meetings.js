...
recordForDelete:function(recordid){
    ...
    // Time field is now optional - no validation required
    ...
    frm.find('#session-rows tr').each(function() {
        ...
        // Only validate if there's any content in the session row
        if (sessionMeetDate || startTime || endTime || topic) {
            // Session date is required if any session data is provided
            if (!sessionMeetDate) {
                errors.push('Session date is required when session details are provided.');
                error_fields.push('input[name="session_meet_date[]"]');
                row.find('input[name="session_meet_date[]"]').addClass("error-field");
            }

            // Session times are now optional - no validation required
            // Topic is now optional - no validation required
        }
    });
