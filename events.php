...
$template_type='';
$page_title = 'Manage Events'.CONST_TITLE_AFX;
$page_description = 'One can manage the events.';
$body_template_file = CONST_THEMES_TEMPLATE_INCLUDE_PATH . 'events.tpl';
...
if(filter_has_var(INPUT_POST,'mode') && $_POST['mode']=='createrec'){
	...
	if($can_add===false){
		...
		if(!isset($data['early_bird']) || $data['early_bird']=='n'){
		...
		$ev_obj = new \eBizIndia\Event();	
		$validation_res = $ev_obj->validate($data, 'add', $other_data);
		if($validation_res['error_code']>0){
			...
			try{
				...
				$error_details_to_log['mode'] = 'createEvent';
				$error_details_to_log['part'] = 'Create a new event.';
				$rec_id=$ev_obj->saveDetails($data);
				if($rec_id===false)
					throw new Exception('Error creating a new Event.');

				$image_names_to_update = [];
				$banner_img_res = $ev_obj->uploadImage($rec_id, $_FILES['dsk_img']['name'], $_FILES['dsk_img']['tmp_name'], 'dsk');
				if(empty($banner_img_res)){
					$result['error_code']=3;
					$result['message']='The Event could not be created as the image for desktop screens could not be uploaded.';
					throw new Exception('Error creating a new Event.');
				}else{
					$image_names_to_update['dsk_img'] = $banner_img_res['gen_file_name'];
				}

				$banner_img_res = $ev_obj->uploadImage($rec_id, $_FILES['mob_img']['name'], $_FILES['mob_img']['tmp_name'], 'mob');
				if(empty($banner_img_res)){
					$result['error_code']=3;
					$result['message']='The Event could not be created as the image for mobile screens could not be uploaded.';
					throw new Exception('Error creating a new Event.');
				}else{
					$image_names_to_update['mob_img'] = $banner_img_res['gen_file_name'];
				}

				if(!$ev_obj->saveDetails($image_names_to_update, $rec_id)){
					unlink(CONST_EVENT_IMG_DIR_PATH.$image_names_to_update['dsk_img']);
					unlink(CONST_EVENT_IMG_DIR_PATH.$image_names_to_update['mob_img']);
					$result['error_code']=4;
					$result['message']='The Event could not be created as the images could not be registered with the event.';
					throw new Exception('Error creating a new Event.');
				}

				$result['error_code']=0;
				$result['message']='The event <b>'.\eBizIndia\_esc($data['name']).'</b> has been created.';
				$conn->commit();
				...
				if($result['error_code']==0){
					...
					$result['message']="The Event could not be created due to server error.";
				}
	...
	echo "parent.evfuncs.handleUpdateRecResponse(".json_encode($_SESSION['update_user_result']).");\n";
	echo "</script>";
	...
	header("Content-Type: text/html; charset=UTF-8");
	echo "<script type='text/javascript' >\n";
	echo "parent.evfuncs.handleAddRecResponse(".json_encode($_SESSION['create_rec_result']).");\n";
	echo "</script>";
	...
	if($_POST['recordid']==''){
		...
		$obj = new \eBizIndia\Event((int)$_POST['recordid']);
	...
	if($recordcount>0){
		...
		$records=\eBizIndia\Event::getList($options);
